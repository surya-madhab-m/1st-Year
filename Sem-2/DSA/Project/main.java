...
class Date
{
	 ...
	 {
		 return this.year < d1.year ||(this.year == d1.year && (getMonthValue(this.month) < getMonthValue(d1.month) || (getMonthValue(this.month) == getMonthValue(d1.month)&& this.day < d1.day)));
	 }
	 public int getDay()
	 {
		 return day;
...
class Member
{
    ...
    public Member(int memberId, String lastName, String firstName, char gender, String memberType, long phone, Date joinDate) 
    {
    ...
    public Member(int memberId, String lastName, String firstName, int handicap, char gender, String memberType, long phone, Date joinDate)
    {
        this(memberId, lastName, firstName, gender, memberType, phone, joinDate);
        this.handicap = handicap;
    }

    public Member(int memberId, String lastName, String firstName, int handicap, char gender, String team, String memberType, long phone, Date joinDate)
    {
        this(memberId, lastName, firstName, handicap, gender, memberType, phone, joinDate);
        this.team = team;
    }

    public Member(int memberId, String lastName, String firstName, int handicap, char gender, String memberType, int coach, long phone, Date joinDate)
    {
        this(memberId, lastName, firstName, handicap, gender, memberType, phone, joinDate);
        this.coach = coach;
    }
...
public class main
{
    ...
    public static void main(String[] args) 
    {
        ...
        {
            if (member.getJoinDate().isBefore(requiredDate)) 
            {
            	member.display();
            }
        }
        ...
        for (Member member : members) 
        {
            if (member.getMemberType().equals("Senior") && (member.getHandicapScore() < 12 && member.getHandicapScore() > 0))
            {
            	member.display();
            }
        }
        ...
        for (Member member : members)
        {
            if (member.getMemberType().equals("Senior") && member.getGender() == 'F' && member.getTeam().equals("TeamB")) 
            {
                member.display();
            }
        }
